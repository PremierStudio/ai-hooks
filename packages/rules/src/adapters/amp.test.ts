...
import { AmpRuleAdapter } from "./amp.js";

describe("AmpRuleAdapter", () => {
  let adapter: AmpRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new AmpRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("amp"));
    it("has correct name", () => expect(adapter.name).toBe("Amp"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".amp/rules"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".amp/rules/test.md");
    });

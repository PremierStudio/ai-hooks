...
import { CodexRuleAdapter } from "./codex.js";

describe("CodexRuleAdapter", () => {
  let adapter: CodexRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new CodexRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("codex"));
    it("has correct name", () => expect(adapter.name).toBe("Codex"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".codex/instructions"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".codex/instructions/test.md");

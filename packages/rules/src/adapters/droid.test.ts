...
import { DroidRuleAdapter } from "./droid.js";

describe("DroidRuleAdapter", () => {
  let adapter: DroidRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new DroidRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("droid"));
    it("has correct name", () => expect(adapter.name).toBe("Droid"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".factory/instructions"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".factory/instructions/test.md");
    });

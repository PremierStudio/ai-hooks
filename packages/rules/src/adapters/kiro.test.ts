...
import { KiroRuleAdapter } from "./kiro.js";

describe("KiroRuleAdapter", () => {
  let adapter: KiroRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new KiroRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("kiro"));
    it("has correct name", () => expect(adapter.name).toBe("Kiro"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".kiro/steering"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".kiro/steering/test.md");
    });

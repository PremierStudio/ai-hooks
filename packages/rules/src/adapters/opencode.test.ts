...
import { OpenCodeRuleAdapter } from "./opencode.js";

describe("OpenCodeRuleAdapter", () => {
  let adapter: OpenCodeRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new OpenCodeRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("opencode"));
    it("has correct name", () => expect(adapter.name).toBe("OpenCode"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".opencode/instructions"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".opencode/instructions/test.md");
    });

...
import { WindsurfRuleAdapter } from "./windsurf.js";

describe("WindsurfRuleAdapter", () => {
  let adapter: WindsurfRuleAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new WindsurfRuleAdapter();
  });

  describe("metadata", () => {
    it("has correct id", () => expect(adapter.id).toBe("windsurf"));
    it("has correct name", () => expect(adapter.name).toBe("Windsurf"));
    it("has correct configDir", () => expect(adapter.configDir).toBe(".windsurf/rules"));
  });
  ...
  describe("generate", () => {
    it("generates files at correct path", async () => {
      ...
      expect(files[0].path).toBe(".windsurf/rules/test.md");
    });

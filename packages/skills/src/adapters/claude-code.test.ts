...
import { ClaudeCodeSkillAdapter } from "./claude-code.js";
...
describe("ClaudeCodeSkillAdapter", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new ClaudeCodeSkillAdapter();
  });
  ...
  describe("metadata", () => {
    ...
    it("has correct config dir", () => expect(adapter.configDir).toBe(".claude/commands"));
  });
  ...
  describe("generate", () => {
    it("generates one file per skill", async () => {
      ...
      expect(files[0]?.path).toBe(".claude/commands/review.md");
  ...
  describe("import", () => {
    ...
    it("imports skill with heading but no blank line after", async () => {
      vi.mocked(existsSync).mockReturnValue(true);
      vi.mocked(readdir).mockResolvedValue(["test.md"] as never);
